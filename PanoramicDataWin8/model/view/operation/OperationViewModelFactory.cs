...
namespace PanoramicDataWin8.model.view.operation
{
    public class OperationViewModelFactory
    {
        ...
        private static void createBottomAttributeGroupMenu(AttributeGroupOperationViewModel attributeGroupOperationViewModel)
        {
            var attachmentViewModel =
                attributeGroupOperationViewModel.AttachementViewModels.First(
                    avm => avm.AttachmentOrientation == AttachmentOrientation.Bottom);
            attachmentViewModel.ShowOnAttributeMove = true;
            ...
            var addMenuItem = new MenuItemViewModel
            {
                ...
                IsWidthBoundToParent = false,
                IsHeightBoundToParent = false,
                Position = attributeGroupOperationViewModel.Position
            };
            var attr1 = new AttributeTransformationMenuItemViewModel
            {
                Label = "+",
            ...
            attr1.DroppedTriggered = attributeTransformationModel => {
                if (!attributeGroupOperationViewModel.AttributeGroupOperationModel.AttributeUsageTransformationModels.Contains(attributeTransformationModel))
                    attributeGroupOperationViewModel.AttributeGroupOperationModel.AttributeUsageTransformationModels.Add(attributeTransformationModel);
            };
            ...
            attributeGroupOperationViewModel.AttributeGroupOperationModel.AttributeUsageTransformationModels.CollectionChanged += (sender, args) =>
            {
                var coll = sender as ObservableCollection<AttributeTransformationModel>;
                ...
                if (args.NewItems != null)
                    foreach (var newItem in args.NewItems)
                    {
                        ...
                        {
                            Label = newAttributeTransformationModel.GetLabel(),
                            AttributeTransformationViewModel = new AttributeTransformationViewModel(attributeGroupOperationViewModel, newAttributeTransformationModel),
                            TextBrush = new SolidColorBrush(Helpers.GetColorFromString("#29aad5")),
                            CanDrag = false,
                            CanDrop = false
                        };
                        newMenuItem.Deleted += (sender1, args1) =>
                        {
                            var atm =
                                ((AttributeTransformationMenuItemViewModel)((MenuItemViewModel)sender1).MenuItemComponentViewModel).AttributeTransformationViewModel.AttributeTransformationModel;
                            attributeGroupOperationViewModel.AttributeGroupOperationModel.AttributeUsageTransformationModels.Remove(atm);
                        };
                        newMenuItem.MenuItemComponentViewModel = newAttr;
                        menuViewModel.MenuItemViewModels.Add(newMenuItem);
                    }
            ...
            attributeGroupOperationViewModel.OperationViewModelTapped += (sender, args) =>
            {
                attachmentViewModel.ActiveStopwatch.Restart();
            };
        ...
        {
            var attributeGroupOperationModel = new AttributeGroupOperationModel(schemaModel);
            var attributeGroupOperationViewModel = new AttributeGroupOperationViewModel(attributeGroupOperationModel);
            attributeGroupOperationViewModel.Position = position;
            addAttachmentViewModels(attributeGroupOperationViewModel);
            createBottomAttributeGroupMenu(attributeGroupOperationViewModel);
            return attributeGroupOperationViewModel;
        }
